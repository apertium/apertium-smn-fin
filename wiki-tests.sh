...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q https://wiki.apertium.org/wiki/North_Sámi_and_Finnish/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch https://wiki.apertium.org/wiki/North_Sámi_and_Finnish/$TESTTYPE"; fi
fi
echo "..."
